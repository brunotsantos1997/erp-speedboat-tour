...
const ClauseItem: React.FC<{ number: number; text: string }> = ({ number, text }) => (
...
export const VoucherScreen: React.FC = () => {
  const { voucher, isLoading, error, handleDownloadPdf } = useVoucherViewModel();
  ...
  if (!voucher) {
    return (
      <div className="flex items-center justify-center h-screen bg-gray-100">
        <p className="text-xl text-gray-600">Voucher não encontrado.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-gray-100 p-4 sm:p-8 font-sans">
      <div id="voucher-content" className="max-w-4xl mx-auto bg-white rounded-lg shadow-2xl">
        {/* Header */}
        <header className="p-6 sm:p-8 bg-gray-800 text-white rounded-t-lg flex justify-between items-center">
          <div className="flex items-center">
            ...
            <div>
              <h1 className="text-xl sm:text-2xl font-bold">Passeio de Lancha</h1>
              <p className="text-xs sm:text-sm text-gray-300">CNPJ: 00.000.000/0001-00 | (21) 98765-4321</p>
            </div>
          </div>
          <button
        ...
        <main className="p-6 sm:p-8">
          ...
          <section className="p-6 bg-gray-50 border border-gray-200 rounded-lg">
            ...
            <ul className="space-y-3">
                <ClauseItem number={1} text="O cancelamento com reembolso de 100% do sinal é permitido apenas se feito com 7 dias de antecedência. Após este período, o sinal não é reembolsável." />
                <ClauseItem number={2} text="Condições climáticas adversas (chuva forte, ventos perigosos) podem levar ao reagendamento do passeio sem custo adicional, a ser combinado entre as partes." />
                <ClauseItem number={3} text="Danos causados à embarcação por mau uso dos passageiros são de responsabilidade do contratante." />
                <ClauseItem number={4} text="O embarque ocorrerá na Marina da Glória, portão B. É recomendado chegar com 15 minutos de antecedência. Tolerância de atraso de 10 minutos." />
            </ul>
          </section>
        </main>
        ...
        <footer className="text-center p-4 text-xs text-gray-500 bg-gray-100 rounded-b-lg">
            ...
            <p>Passeio de Lancha &copy; {new Date().getFullYear()}</p>
        </footer>
      </div>
    </div>
  );
