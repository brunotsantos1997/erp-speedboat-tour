...
test('Capture application screenshots no tutorial', async ({ page }) => {
  ...
  // Skip dashboard tutorial before screenshot
  ...
  for (const screen of screens) {
    ...
    // Create minimal data if screen is empty to make a better print
    if (screen.path === '/dashboard/boats') {
        if (await page.locator('text=Nenhuma lancha cadastrada').isVisible()) {
