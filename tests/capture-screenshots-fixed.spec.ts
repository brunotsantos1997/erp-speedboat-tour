...
test('Capture application screenshots fixed', async ({ page }) => {
  ...
  // Wait for dashboard and ensure it's not the login page
  await page.waitForURL('**/dashboard', { timeout: 15000 });
  await page.waitForLoadState('networkidle');
  await page.waitForTimeout(3000); // Give time for data to populate
  ...
  if (await viewVoucherBtn.isVisible()) {
      ...
      await page.waitForURL('**/voucher/**', { timeout: 10000 });
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(2000);
